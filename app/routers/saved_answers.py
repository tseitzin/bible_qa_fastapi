...
async def save_answer(data: SavedAnswerCreate, current_user: CurrentUser):
    """Save an answer to user's collection."""
    try:
        SavedAnswersRepository.save_answer(user_id=current_user["id"], question_id=data.question_id, tags=data.tags)

        # Fetch the complete saved answer details
        saved_answers = SavedAnswersRepository.get_user_saved_answers(user_id=current_user["id"], limit=1)

        if not saved_answers:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve saved answer"
            )
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to save answer")
    ...
    try:
        if query or tag:
            saved_answers = SavedAnswersRepository.search_saved_answers(
                user_id=current_user["id"], query=query, tag=tag
            )
        else:
            saved_answers = SavedAnswersRepository.get_user_saved_answers(user_id=current_user["id"], limit=limit)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve saved answers"
        )


@router.delete("/{saved_answer_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_saved_answer(saved_answer_id: int, current_user: CurrentUser):
    """Delete a saved answer from user's collection."""
    try:
        deleted = SavedAnswersRepository.delete_saved_answer(
            user_id=current_user["id"], saved_answer_id=saved_answer_id
        )
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete saved answer")
